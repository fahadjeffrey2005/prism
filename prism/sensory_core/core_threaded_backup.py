...
class DepthEstimator:
    ...
    INFERENCE_SIZE = (448, 252)   # width, height — M4 sweet spot
...
class SensoryCore:
    ...
    def process(
        ...
        # Detection + Depth run in parallel — they're independent operations
        import threading
        depth_result = [None]
        ...
        def run_depth():
            if schedule["depth"]:
                with Timer("depth") as t:
                    depth_result[0] = self.depth_estimator.estimate(image)
                depth_time[0] = round(t.elapsed * 1000, 1)

        def run_detection():
            if schedule["detection"]:
                with Timer("detection") as t:
                    detection_result[0] = self.detector.detect(image)
                detection_time[0] = round(t.elapsed * 1000, 1)

        t_depth = threading.Thread(target=run_depth)
        ...
        if depth_result[0] is not None:
            frame.depth_map = depth_result[0]
            processing_times["depth_ms"] = depth_time[0]
        ...
        if detection_result[0] is not None:
            ...
            processing_times["detection_ms"] = detection_time[0]
