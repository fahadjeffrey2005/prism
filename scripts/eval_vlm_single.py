...
for scene_idx in [8, 9]:
    ...
    ped_count = sum(1 for a in sample["anns"] if "pedestrian" in nusc.get("sample_annotation", a)["category_name"])
    veh_count = sum(1 for a in sample["anns"] if "vehicle" in nusc.get("sample_annotation", a)["category_name"])

    messages = [{"role": "user", "content": [{"type": "image", "image": image}, {"type": "text", "text": COT_PROMPT}]}]
    ...
    try:
        ...
        if start >= 0 and end > start:
            ...
            print("RESULT: peds=" + str(parsed.get("total_pedestrians")) +
                  " veh=" + str(parsed.get("total_vehicles")) +
                  " decision=" + str(parsed.get("decision")) +
                  " risk=" + str(parsed.get("primary_risk")))
            print("Ped error: " + str(abs(parsed.get("total_pedestrians", 0) - ped_count)) + " (GT=" + str(ped_count) + ")")
